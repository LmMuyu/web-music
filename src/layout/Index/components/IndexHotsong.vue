  ...
  >
    <template v-slot:left>
      ...
      >
        <ElRow
          class="w-full py-2"
          v-for="(song, songindex) in leftSongs"
          :key="songindex"
        >
          <ElCol :span="5">
            <ElAvatar
              shape="square"
              :src="song.picUrl + '?param=42y42'"
              :size="42"
            ></ElAvatar>
          </ElCol>
    ...
    <template v-slot:midd>
      ...
      >
        <ElRow
          class="w-full py-2"
          v-for="(song, songindex) in middSongs"
          :key="songindex"
        >
          <ElCol :span="5">
            <ElAvatar
    ...
    <template v-slot:right>
      ...
      >
        <ElRow
          class="w-full py-2"
          v-for="(song, songindex) in rightSongs"
          :key="songindex"
        >
          <ElCol :span="5">
            <ElAvatar
...
newTopSongs.value = (await topSong()).data.data.map((song) =>
  musicResultDetail(song)
);
...
function songSlice(sliceindex: number) {
  ...
  return [
    newTopSongs.value.slice(
      SLICE_COUNT * sliceindex,
      SLICE_COUNT * (sliceindex + 1)
    ),
    newTopSongs.value.slice(
      SLICE_COUNT * (sliceindex + 1),
      SLICE_COUNT * (sliceindex + 2)
    ),
  ];
}
...
function useSongsLists(pageindex: Ref<number>) {
  ...
  watchEffect(() => {
    ...
    rights.value =
      sliceArr[2] > newTopSongs.value.length - 1 ? [] : songSlice(sliceArr[2]);
  });
